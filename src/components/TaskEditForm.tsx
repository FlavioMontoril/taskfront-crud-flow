...
export function TaskEditForm({ task }: FindTaskProps) {
    console.log("TUTU", task)
    const navigate = useNavigate()
    const { taskApi } = useApi()
    const { updateTask } = useTaskStore()
    const { id } = useParams()

    const { register, handleSubmit, formState: { errors, isSubmitting }, } = useForm<UpdatedTaskSchema>({
